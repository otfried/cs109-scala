...
class Date private (val year: Int, val month: Int, val day: Int) {
  ...
  def +(n: Int): Date = Date.fromDayIndex(dayIndex + n)
  def -(n: Int): Date = Date.fromDayIndex(dayIndex - n)
}
